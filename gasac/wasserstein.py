...
class WGASACAgent:
    def __init__(self, state_size, action_size, nf_archi, buffer_size=int(1e6), batch_size=256, balance=1.0,
    ...
    def update_network(self):
        ...
        loss_D_pi = torch.mean(self.discriminator(states[:64], action_G))
        ...
        loss_D_IS = self.discriminator(states[:64], action_D)
        ...
        if self.it % 4 == 0:
            ...
            loss_pi_implicit = - self.balance * torch.mean(self.discriminator(states[:64], actions_G))
    ...
    def save_model(self, file_name):
        save_path = f"./weights_WassersteinGASAC_{self.nf_archi}"
